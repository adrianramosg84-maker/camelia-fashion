...
export function Checkout({ total, items, onBack, onSuccess }: CheckoutProps) {
  ...
  useEffect(() => {
    ...
    if (paymentConfig.accountNumber) setPaymentMethod('transfer');
    else if (paymentConfig.yapeNumber) setPaymentMethod('yape');
    else if (paymentConfig.plinNumber) setPaymentMethod('plin');
    else if (paymentConfig.paypalEmail) setPaymentMethod('paypal');
  }, [paymentConfig]);
  ...
  const buildWhatsappMessage = (order: Order) => {
    const itemsList = order.items
      .map((i) => `• ${i.name} x${i.quantity} — S/ ${((i.price ?? 0) * i.quantity).toFixed(2)}`)
      .join('\n');
    return (
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const savedOrders: Order[] = JSON.parse(
      localStorage.getItem('camelia-orders') ?? '[]'
    );
    savedOrders.unshift(order);
    localStorage.setItem('camelia-orders', JSON.stringify(savedOrders));
  ...
  if (orderConfirmed) {
    ...
    return (
      <div className="h-full flex flex-col items-center justify-center p-8 text-center gap-6">
        ...
        <div className="w-full bg-secondary/40 rounded-lg p-4 text-left text-sm space-y-2">
          {paymentMethod === 'transfer' && paymentConfig && (
            <>
              ...
              <p>
                <strong>Tipo:</strong>{' '}
                {paymentConfig.accountType === 'ahorros' ? 'Ahorros' : 'Corriente'}
              </p>
              <p><strong>Cuenta:</strong> {paymentConfig.accountNumber}</p>
              <p className="pt-2 border-t border-border">
                <strong>Monto:</strong>{' '}
                <span className="text-primary text-base">S/ {orderConfirmed.total.toFixed(2)}</span>
              </p>
            </>
          )}
          {paymentMethod === 'yape' && paymentConfig && (
            <>
              ...
              {paymentConfig.yapeQR && (
                <img
                  src={paymentConfig.yapeQR}
                  alt="QR Yape"
          ...
          {paymentMethod === 'plin' && paymentConfig && (
            <>
              ...
              {paymentConfig.plinQR && (
                <img
                  src={paymentConfig.plinQR}
                  alt="QR Plin"
                  className="w-36 h-36 mx-auto mb-2 object-contain"
                />
              )}
        ...
        <button
          onClick={onSuccess}
          className="w-full bg-primary text-primary-foreground py-3 rounded-md hover:opacity-90 transition-all"
        >
          Volver a la tienda
        </button>
      </div>
    ...
    return (
      <div className="h-full flex flex-col items-center justify-center p-8 text-center gap-4">
        ...
        <p className="text-sm text-muted-foreground">
          El dueño de la tienda aún no ha configurado los métodos de pago. Por favor, intenta más tarde
          o contácta directamente a la tienda.
        </p>
        <button
          onClick={onBack}
  ...
  return (
    <div className="h-full overflow-y-auto">
      ...
      <form onSubmit={handleSubmit} className="p-6 space-y-6">
        ...
        {paymentMethod === 'yape' && paymentConfig && (
          <div className="bg-secondary/30 p-4 rounded-lg text-center text-sm">
            <h4 className="font-semibold mb-2">Número Yape: {paymentConfig.yapeNumber}</h4>
            {paymentConfig.yapeQR && (
              <img src={paymentConfig.yapeQR} alt="QR Yape" className="w-32 h-32 mx-auto object-contain" />
            )}
          </div>
        )}
        {paymentMethod === 'plin' && paymentConfig && (
          <div className="bg-secondary/30 p-4 rounded-lg text-center text-sm">
            <h4 className="font-semibold mb-2">Número Plin: {paymentConfig.plinNumber}</h4>
            {paymentConfig.plinQR && (
              <img src={paymentConfig.plinQR} alt="QR Plin" className="w-32 h-32 mx-auto object-contain" />
            )}
          </div>
        )}
        {paymentMethod === 'paypal' && paymentConfig && (
          <div className="bg-secondary/30 p-4 rounded-lg text-sm">
            <p><strong>Email PayPal:</strong> {paymentConfig.paypalEmail}</p>
          </div>
        )}
        ...
        <div className="pt-4 border-t border-border">
          <button
            ...
            className="w-full bg-primary text-primary-foreground py-3 rounded-md hover:opacity-90 transition-all hover:shadow-md flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {isProcessing ? (
              <>
